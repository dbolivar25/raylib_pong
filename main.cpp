...
struct Paddle {
    ...
    Paddle(Rectangle rect, Vector2 velocity) : m_rect(rect), m_velocity(velocity) {}
};
...
int main() {
    ...
    Camera2D camera = { { screenWidth/2.0f, screenHeight/2.0f }, { screenWidth/2.0f, screenHeight/2.0f }, 0, 1 };
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC// key
    {
        ...
        if (topCollision) {
            ...
            // reset the ball to the center of the screen and give it a random velocity
            ball.m_rect.x = screenWidth / 2 - ball.m_rect.width / 2;
            ball.m_rect.y = screenHeight / 2 - ball.m_rect.height / 2;
        ...
        if (CheckCollisionRecs(ball.m_rect, left) || CheckCollisionRecs(ball.m_rect, right)) {
            ball.m_velocity.x *= -1;
        }
        ...
        if (paddle1Collision || paddle2Collision) {
            ...
            double contactPoint = ball.m_rect.x - (paddle1Collision ? paddle1.m_rect.x : paddle2
                    .m_rect.x);

            ball.m_velocity.x = std::min((contactPoint - (paddle1.m_rect.width / 2)) / 10, 4.0);
        }
        ...
        BeginDrawing();
            ...
            BeginMode2D(camera);
                ...
                // draw the score board on the right side of the screen with the score for score[0] on
                // the bottom and score[1] on the top
                DrawText(std::to_string(score[1]).c_str(), screenWidth - 50, (screenHeight / 2) -
                50,20, BLACK);
                DrawText(std::to_string(score[0]).c_str(), screenWidth - 50, (screenHeight / 2)
                + 50, 20, BLACK);
